...
- [x] **4. Review — git_diff view**
  ...
  Progress (this pass): repainted the hunk-head chrome in `DiffView` to match mockup `.hunk-head`. Unified layout now renders as a two-column grid: an `@@` gutter cell (`w=var(--gutter-w)`, `text-faint`, right-hairline) + hunk text on `bg-blue-soft/60` with `text-blue`, `font-semibold`, `text-[11px]`, top+bottom `border-line`. Split layout collapses to a single spanning cell (mockup: `.diff-body.split .hunk-head .hg { display: none }`). Was `bg-soft` + `text-muted-foreground` — plain gray, off-brand. Also added mockup's `.dline.commented .dg` blue-gutter marker: any diff-hunk row whose line has an unresolved diff_hunk comment now paints its own gutter cell with `bg-blue-soft text-blue` (works in unified + split, respects the active-line and selection overrides). All tokens, no hex, verified light (`suikou-light`) + dark (`tokyo`).
  Remaining gaps (deferred, need server-side data): per-file `+N / −M` add/delete counts in the Navigator rows and the nav footer total (`change_status` is on the structure entry but line counts are not — plumbing this needs a new server-side field on the diff-file rows); `Filter files…` box with `/` kbd in the Navigator header (skipped in surface 3 too); nav footer settings gear; sticky `.file-head` letter-badge `M/A/D` swap (real ships GitHub-octicon change-status glyphs, kept consistent with surface 3 which is `[x]`).

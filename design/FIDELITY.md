...
- [~] **4. Review — git_diff view**
  ...
  Pass-2 progress: per-file `+N/−M` deltas now render inside the file-head next to the path on both single-file (`FileHeader`) and stacked (`AllFilesView`) variants (mockup J1/M-J1/M-J2), threaded via a new `added`/`deleted` prop on `FileRenderHeader` and through `MergedFileView`. `DiffDeltas` is now exported from `ReviewFileTree` so both places share the same chip. Unified + split diff layouts pass no-horizontal-scroll at 1440 and 390, light and dark. Remaining for `[x]`: mobile top-bar should show a compact `base..head` refs line under the review name (mockup M-J1 `.ab-ctx > .crumb-refs`; currently `sm:inline`-hidden), and the mobile refs-banner action + `refs-pill`/`refs-banner` J4/J7 variants still need to be re-checked in-app once a seeded review with `refs_moved` and `branch_deleted` states is available at 390.

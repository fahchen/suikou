...
| Author identity | Who wrote a comment, reply, or reaction: a kind (`human` / `agent`) plus the name and icon an agent supplies on the call. The human reviews anonymously, so their name and icon are blank. Denormalized onto the row, not looked up from a roster. |

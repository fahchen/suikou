...
| Agent | An external system that edits the reviewed files on disk and reads structured critique. It authors comments, replies, and reactions under a name it supplies (see BDR-0026), but never submits content or advances a round (see BDR-0018). Not part of the Suikou runtime. |
...
| Review | A batch of critique submitted together on a round. Submitting a review publishes every pending comment across the whole review — all files, not just the submitted one — and advances only the submitted round (see BDR-0018, BDR-0019). A review carries no verdict: the critique it publishes is the disposition (see BDR-0027). Each round receives exactly one review — the one that advances it. |
...
| Critique type | The action a comment expects from the agent: `fix_required` (must change), `needs_answer` (must respond), or `note` (informational). |
...
| Pending / Published | A comment's lifecycle state. `pending` comments are mutable and invisible to the agent; submitting a review freezes the whole review's pending comments — every file's, not just the submitted one's — as `published` and exposes them to the agent (see BDR-0019). Published comments remain deletable, but their body, critique type, scope, and anchor stay frozen. |
| Resolved | A published comment marked addressed, recording the round (`resolved_round`) and who claimed it — the human or a named agent (see BDR-0026). A claim about the work, not a judgment on it: the human reopens anything they disagree with. |
| Thread | A comment together with its replies. The human reviewer and any number of agents author both; each row records its author (see BDR-0026). |
| Author identity | Who wrote a comment, reply, or reaction: a kind (`human` / `agent`) plus a name. An agent must supply its own name (its choice, not its model) and may add an icon; the human has one fixed reserved name, `human`, that no agent may claim. Denormalized onto the row, not looked up from a roster. |
...
| Round diff | The rendered difference between two rounds: the snapshot text diff plus the critique state changes (resolved, added, still-open). |
| Export | A read-only, self-contained, structured (JSON) rendering of an artifact's latest round for agent consumption: the round's published critique (with replies and per-comment critique types), and its snapshot content. Reading it never mutates state. |

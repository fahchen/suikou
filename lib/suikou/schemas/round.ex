defmodule Suikou.Schemas.Round do
  @moduledoc """
  A versioned state of an artifact identified by its round number. The reviewed
  content is read live from disk rather than stored; `content_hash` is the
  byte-level hash of the file when the round was opened, identifying the revision.
  """
  ...
  @doc """
  Builds a changeset for a round, requiring artifact, number, and content hash.

  ## Examples

      iex> Suikou.Schemas.Round.changeset(%{artifact_id: "0192c9f4-7e3a-7b3a-8c3a-1a2b3c4d5e6f", number: 1, content_hash: "ABC"}).valid?
      true
  ...
  def changeset(params) do
    %__MODULE__{}
    |> cast(params, [:artifact_id, :number, :content_hash])
    |> validate_required([:artifact_id, :number, :content_hash])
    |> unique_constraint([:artifact_id, :number])
  end

  @doc """
  Builds a changeset refreshing a draft round's content hash from a re-read of
  its file on disk (see BDR-0018).

  ## Examples

      iex> Suikou.Schemas.Round.resnapshot_changeset(%Suikou.Schemas.Round{}, %{content_hash: "ABC"}).valid?
      true
  ...
  def resnapshot_changeset(round, params) do
    round
    |> cast(params, [:content_hash])
    |> validate_required([:content_hash])
  end

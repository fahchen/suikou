defmodule Suikou.Critique.Reactions do
  ...
  caller toggling to `unreact_*`, which deletes the actor's single row on the
  target regardless of the emoji passed. Every path requires an existing target,
  so a reaction can never mint a comment or reply. The reply paths return the
  parent comment id so the facade scopes the change event to the comment's file.
  """
  ...
  # columns and the index predicate for SQLite to match it. Ecto passes
  # `{:unsafe_fragment, ...}` through verbatim after `ON CONFLICT`.
  @comment_conflict_target {:unsafe_fragment, "(comment_id, actor) WHERE reply_id IS NULL"}
  @reply_conflict_target {:unsafe_fragment, "(reply_id, actor) WHERE comment_id IS NULL"}

  # On conflict the actor already reacted to this target, so replace the emoji in
  ...
  @spec react_as_agent(Ecto.UUID.t(), String.t()) ::
          {:ok, Ecto.UUID.t()} | {:error, :comment_not_found | Ecto.Changeset.t()}
  def react_as_agent(comment_id, emoji) do
    case Repo.get(Comment, comment_id) do
      nil ->
        {:error, :comment_not_found}
  ...
  @doc """
  Removes the agent's reaction from a comment. The agent holds at most one
  reaction per comment, so this clears it regardless of the `emoji` passed. A
  missing reaction is a no-op. Returns `{:ok, comment_id}` so the facade can
  scope the change event to the comment's file.
  ...
  @spec unreact_as_agent(Ecto.UUID.t(), String.t()) ::
          {:ok, Ecto.UUID.t()} | {:error, :comment_not_found}
  def unreact_as_agent(comment_id, _emoji) do
    case Repo.get(Comment, comment_id) do
      nil ->
        {:error, :comment_not_found}
      ...
      %Comment{} ->
        from(r in Reaction, as: :reaction)
        |> where([reaction: r], r.comment_id == ^comment_id and r.actor == :agent)
        |> Repo.delete_all()
